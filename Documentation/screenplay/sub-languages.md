...
The executable semantic compiler does not run inline code or file-backed implementations. Reducer transitions with bodies, named rule predicates, command and concept code validations, and inline or file-backed policy predicates bind as opaque ESM v3 attachments. The reference executor cannot run them: execution that depends on their outcomes returns Unsupported. Other code roles, such as command handlers, still block executable admission with `PLAY0268`. `ImplementationRequirements` lists each attachment's RequirementId, owning semantic address and role (with a member discriminator where needed), context and result contract versions, RequiredCapability, AttachmentResolution, inline language or authored file path, and source-map location. The SHA-256 content hash is present only for inline code or file contents supplied by the host. File attachments without host-supplied contents are reported unresolved, never hashed from the path; the binder does not open files. Hosts with a trusted model root can use `AttachmentFiles.Load(root, documents)` and supply the result to `SemanticDocumentSet.Create` or an in-memory workspace's `WithAttachmentContents`. Only implementation attachments are read, with confined access and warning diagnostics for refusals ([#244](https://github.com/Cratis/Screenplay/issues/244)). `PlayFileCompiler` and the standalone Tool compile syntax only and do not read attachments.

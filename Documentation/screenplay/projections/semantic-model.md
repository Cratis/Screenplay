...
`new SemanticEvaluator().EstablishWorld(plan, facts)` takes a capability-admitted `SemanticExecutionPlan` and an ordered `ImmutableArray<SemanticFact>`. It validates the entire fact history against the plan's event contracts before projecting it, then returns a `SemanticAccepted` whose `World.Facts` preserves occurrence order and whose `World.ReadModels` contains derived keyed instances in deterministic semantic-identity/key order. The accepted result carries the supplied facts and no query results. An empty array establishes an empty world. A default array, unknown event, malformed occurrence or payload returns `SemanticRejected` with category `Contract` and an unchanged empty world. ESM v2/v3 facts require a nonempty typed event-source identity matching the event's unambiguous declared producer destination type; ESM v1 facts may omit occurrence context; when a v1 fact supplies one, it must match the declared producer destination type too. Every tag must be nonempty text. A projection failure returns `SemanticUnsupported` with capability `Projection`. The public operation also returns `Unsupported` when a supplied event affects a reducer-backed read model: reducer transitions are opaque and cannot be replayed by the reference evaluator. Authored `given` events in specification runs share the validation and projection core, but retain two compatibility rules: a given event without an event source keeps its legacy null destination even in v2/v3, and observing an event alone does not make a specification unsupported unless it asserts or queries reducer-backed state. Supplied event sources still undergo declared-type validation. Explicit given read-model overrides are applied after establishment.

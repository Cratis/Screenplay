...
A join never creates an instance. At the root, when a joined event arrives, every existing instance whose `on` property equals the joined event's source is updated. Inside `children`, the child's identity is compared to the join key instead of `on`. When a `from` event sets that property, the latest joined event is read again, so a customer registered before the order still names it. The identifier written directly after `join` is not part of the definition - Chronicle's lowering discards it.

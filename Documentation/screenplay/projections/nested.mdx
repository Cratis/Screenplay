...
`clear with` takes an event and drops the whole nested object. To clear a single property instead, use the `clear <property>` mapping line inside a `from` block — see [Property Mapping](property-mapping.md#clearing-a-value).
...
The nested property maps to a nullable property on the parent read model. For the `command` example above, the read model declares a nullable `Command` property of a complex type carrying `Name` and `Schema` — it is `null` until the `from` event fires, and set back to `null` by the `clear with` event.
...
2. **Single responsibility**: Model the nested type around one coherent concept; if it needs its own lifecycle or collection behavior, use [children](children) instead

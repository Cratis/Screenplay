...
Access event metadata using `$eventContext.{path}`, a dotted path into the event context:
...
- `sequenceNumber` - Position of the event in its event sequence
- `correlationId` - Correlation ID for tracking related operations
- `eventSourceId` - The event source identifier
- `causedBy.subject`, `causedBy.name`, `causedBy.userName` - The identity that caused the event

The full list, and the paths the compiler checks against, is in [Event Context](event-context.md#available-properties).
...
It resolves the same value as `$eventContext.eventSourceId`, but it is its own expression, not a shorthand for that path - see [Event Context](event-context.md#eventsourceid-and-eventcontexteventsourceid).
...
Access the identity that caused the event using `$eventContext.causedBy`:

```pdl
$eventContext.causedBy.subject
$eventContext.causedBy.name
$eventContext.causedBy.userName
```
...
- `onBehalfOf` - The identity this one acted on behalf of, with the same members

**Examples:**

```pdl
from OrderPlaced
  CreatedBySubject = $eventContext.causedBy.subject
  CreatedByName = $eventContext.causedBy.name
  CreatedByUser = $eventContext.causedBy.userName

from DocumentUpdated
  UpdatedBy = `${$eventContext.causedBy.name} (${$eventContext.causedBy.userName})`
  UpdatedById = $eventContext.causedBy.subject
```
...
To remove a value, prefer [`clear`](property-mapping.md#clearing-a-value) — it names the act rather than assigning nothing. The `null` literal keeps working wherever it is written today.
...
The declaration syntax is transformed when compiled into engine-level projection definitions:
...
This transformation ensures compatibility with the internal expression format of the [Chronicle projections](/chronicle/projections/) engine while keeping the declaration syntax clean and intuitive.

## Type Compatibility

Expressions must be compatible with the target property type. Consider a read model with `Name` (string), `LoginCount` (number), `IsActive` (boolean), and `CreatedAt` (timestamp):
...
```pdl
children <collection> identified by <expression>
```

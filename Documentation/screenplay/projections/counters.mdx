...
Counters track how many times events occur or increment/decrement numeric values. The language provides three counter operations: `count`, `increment`, and `decrement`.
...
Counter properties should have an initial value. Declare counter targets such as `LoginCount` or `ProfileViews` as numeric properties on the read model, starting at `0`, so the first counter operation produces `1`.

...
```pdl
children departments identified by deptId
  from DepartmentCreated key deptId
    parent companyId
    Name = name

  children teams identified by teamId
...
projection Invoice => InvoiceReadModel
  ...
  children payments identified by paymentId
...
projection BlogPost => BlogPostReadModel
  ...
  children comments identified by commentId
...
The children block maps to a collection property on the read model. For the `members` example above, the read model declares a `Members` collection where each item carries the child's properties — `UserId`, `Name`, and `Role` — and the `identified by` expression determines which item in the collection an event applies to.
...
```pdl
children versions identified by versionNumber

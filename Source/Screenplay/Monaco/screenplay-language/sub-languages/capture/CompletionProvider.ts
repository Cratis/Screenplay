...
export class CompletionProvider implements languages.CompletionItemProvider {
    ...
    // Per source type — no `auth`/`bearer`/`url`: an API source references a named
    // External Service (`api <ServiceName>`) for base URL and auth; a webhook source's
    // authentication is configured on the source builder, not in CDL text.
    private addSourceBlockCompletions(suggestions: languages.CompletionItem[], context: CompletionContext, sourceType: string | null): void {
        const options: Record<string, Array<{ label: string; detail: string; documentation: string }>> = {
            api: [
                { label: 'api', detail: 'api <ServiceName>', documentation: 'References a configured External Service by name for the base URL and authentication' },
                { label: 'route', detail: 'route <path>', documentation: 'Optional path appended to the External Service base URL; if omitted, the base URL is used as-is' },

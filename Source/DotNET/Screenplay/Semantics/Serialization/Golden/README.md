...
For a v3-only or v4-only regeneration, use `SCREENPLAY_REGENERATE_GOLDEN=3` or `SCREENPLAY_REGENERATE_GOLDEN=4` with the same command. The `1` setting rewrites all six files; all modes fail on purpose with

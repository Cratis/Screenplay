...
public static class golden_vector_regeneration
{
    ...
    /// </summary>
    public static bool IsRequested => Environment.GetEnvironmentVariable(Variable) == "1" || Environment.GetEnvironmentVariable(Variable) == "3" || Environment.GetEnvironmentVariable(Variable) == "4";
    ...
    public static void RegenerateWhenRequested()
    {
        ...
        if (Environment.GetEnvironmentVariable(Variable) == "4")
        {
            var name = Write(directory, "full-esm-v4.json", SemanticModelSerializer.Serialize(canonical_serialization_golden_vectors.CreateSemanticModelV4()));

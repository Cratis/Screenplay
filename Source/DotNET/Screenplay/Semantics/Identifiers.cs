...
public readonly record struct EventContractId
{
    ...
    /// <summary>
    /// Creates the deterministic legacy bootstrap identity for an event address.
    /// </summary>
    /// <param name="address">The location-independent address of the event declaration.</param>
    /// <returns>The deterministic event contract identity.</returns>
    public static EventContractId CreateLegacy(SemanticAddress address)
    {
        if (address is null)
        {
            throw new InvalidSemanticContract("A legacy event contract identity requires an address.");
        }

        return new(IdentityHash.CreateEvent(address));
    }
...
public readonly record struct SemanticRevision
{
    ...
    /// <summary>
    /// Computes a revision from canonical bytes that omit the revision field.
    /// </summary>
    /// <param name="canonicalBytes">The canonical bytes.</param>
    /// <returns>The computed revision.</returns>
    public static SemanticRevision Compute(ReadOnlySpan<byte> canonicalBytes) =>
        new(Prefix + IdentityText.ToLowerHex(SHA256.HashData(canonicalBytes)));
...
static class IdentityHash
{
    ...
    internal static string CreateEvent(SemanticAddress address)
    {
        if (address.Kind != SemanticKind.EventContract)
        {
            throw new InvalidSemanticContract("A legacy event identity requires an event contract address.");
        }

        return Create(IdentityText.EventPrefix, "event-contract", checked((uint)address.Kind), address.Parts);
    }

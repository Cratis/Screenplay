...
public class when_replacing_a_tag_literal_with_preserved_trivia : Specification
{
    ...
    void Because()
    {
        var entry = WorkspaceSyntaxIndex.Create(_workspace).Entries.Single(entry => entry.Node is LiteralExpressionSyntax literal && literal.Value is string value && string.Equals(value, "audit", StringComparison.Ordinal));

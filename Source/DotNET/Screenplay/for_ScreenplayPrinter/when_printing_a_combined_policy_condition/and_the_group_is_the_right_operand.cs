...
public class and_the_group_is_the_right_operand : for_ScreenplayPrinter.given.a_printer
{
    const string Source =
        """
        policy CanApprove
          require role "Approver" or (role "Manager" and claim "scope" matches "finance")
        """;

    const string Shape = "(role:Approver Or (role:Manager And claim:scope))";
    ...
    [Fact] void should_group_the_right_operand_when_parsing() => Describe(Condition(_roundtrip.Original!)).ShouldEqual(Shape);
    [Fact] void should_print_the_group() => _roundtrip.Printed.ShouldContain(@"require role ""Approver"" or (role ""Manager"" and claim ""scope"" matches ""finance"")");

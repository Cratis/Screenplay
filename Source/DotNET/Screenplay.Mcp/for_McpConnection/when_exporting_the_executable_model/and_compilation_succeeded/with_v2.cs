...
public class with_v2 : given.an_export
{
    System.Text.Json.JsonElement _first;
    byte[] _bytes = null!;
    bool _aligned;

    void Establish() => SetupVersion(2);
    ...
    [Fact] void should_report_schema_version() => _first.GetProperty("schemaVersion").GetInt32().ShouldEqual(2);
    [Fact] void should_report_language_version() => _first.GetProperty("languageVersion").GetString().ShouldEqual("2.0");
    [Fact] void should_report_semantic_version() => _first.GetProperty("semanticVersion").GetString().ShouldEqual("2.0");

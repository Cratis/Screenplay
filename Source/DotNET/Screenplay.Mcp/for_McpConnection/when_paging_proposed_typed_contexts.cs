...
public class when_paging_proposed_typed_contexts : given.a_connection
{
    ...
    [Fact] void should_page_the_proposed_handler_context() => _first.GetProperty("result").GetProperty("page").GetProperty("items")[0].GetProperty("role").GetString().ShouldEqual("CommandHandler");
    [Fact] void should_mark_the_proposed_context_as_unavailable() => _first.GetProperty("result").GetProperty("available").GetBoolean().ShouldBeFalse();

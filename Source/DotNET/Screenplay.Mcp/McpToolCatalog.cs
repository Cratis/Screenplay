...
static class McpToolCatalog
{
    ...
    [
        ...
        new("read-workspace", "Page documents, identities, diagnostics, repairs, source map, implementation requirements or canonical executable model bytes. Executable model and attachment pages require pinned model/manifest revisions on continuation; unavailable models point to executable diagnostics. Read-only; not an ESM edit path.", ["expectedRevision"], ["expectedRevision", "view", "offset", "limit", "expectedModelRevision", "expectedAttachmentManifestRevision"]),
        ...
        new("propose-repair", "Preview one compiler-authored diagnostic repair as a retained, revision-checked typed AST proposal. Discover code and subject with read-workspace view=repairs; review with read-proposal before explicit apply. The identity replacement canonically reprints the entire touched document (including other legacy forms and whitespace); refuses any dropped comment. Requires canonical formatting consent; never writes on discovery or proposal.", ["expectedRevision", "expectedCatalogRevision", "diagnosticCode", "subject", "formatting"], ["expectedRevision", "expectedCatalogRevision", "diagnosticCode", "subject", "formatting", "includeContent"]),
